...
##get the event id that we are looking for based on the event names
...
for event in response["data"]:
	if event["name"] == event_name:
		...
		if event["name"] == triage_event_name:
...
if debug:
	log.write("** DEBUG: URL for request is :{}\n".format(url))
		#Added by ATW on 9/28/17
		#else log.write("** DEBUG: No group guid Found!\n")
response = rest_methods.ampget(url)
